...
try:
    ...
    # 💡 [최적화] DB를 거치지 않고 캐시 메모리에서 프로필 꺼내오기
    ...
    if not my_mentees:
        ...
        for match in my_mentees:
            max_id = match["mentee_id"]
            mentee_profile = next((p for p in all_profiles if p["student_id"] == max_id and p["role"] == "멘티"), {})
            ...
            with st.container(border=True):
                ...
                with col1:
                    ...
                    st.write(f"**9️⃣ 학번:** {max_id} | **👤 이름:** {mentee_name} {alert}")
